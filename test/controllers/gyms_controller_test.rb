...
class GymsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create gym" do
    assert_difference('Gym.count') do
      post gyms_url, params: { gym: { description: @gym.description, email: @gym.email, logo: @gym.logo, name: @gym.name } }
    end
  ...
  test "should update gym" do
    patch gym_url(@gym), params: { gym: { description: @gym.description, email: @gym.email, logo: @gym.logo, name: @gym.name } }
